...
export default function AdminDashboard() {
  ...
  const loadDashboardData = async () => {
    setLoading(true)
    try {
      const [projectsRes, builderRes, clientRes, requestsRes] = await Promise.all([
        api.get<Project[]>('/api/projects/').catch(() => ({ data: [] })),
        api.get<{ count: number }>('/api/users/builders/count').catch(() => ({ data: { count: 0 } })),
        api.get<{ count: number }>('/api/users/clients/count').catch(() => ({ data: { count: 0 } })),
        api.get<{ count: number }>('/api/admin/requests/pending/count').catch(() => ({ data: { count: 0 } })),
      ])
    ...
    <div className="admin-dashboard-page">
      ...
      <div className="admin-stats-grid">
        ...
        <Card className="admin-stat-card">
          ...
          <div className="admin-stat-value">{stats.completedProjects}</div>
        ...
        <Card className="admin-stat-card">
          ...
          <div className="admin-stat-value">{stats.totalBuilders}</div>
